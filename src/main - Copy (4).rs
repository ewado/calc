...
impl CalState {
	...
	fn display_results_expressions(&self) {
		...
		for (i, (r, e)) in iter {
			...
			//println!("#{:0>2}  {}{}{}{}{}", i , result_str, spaces_after_result, printing_x, spaces_after_x, e);
...
//			&Input::Add => {
	...
	fn recreate(&mut self) {
		...
		let mut prev_action: Input = Input::Add; // Input::String("$".to_string());     //use Option
...
/* 			match action {
				Input::Add | Input::Subtract => {
					match prev_action {
						...
						Input::Add | Input::Subtract => {with_brackets = false;},
						
						_ => {with_brackets = true;},
						//_ => {with_brackets = false;}
					}
				},
				Input::Multiply | Input::Divide => {
					match prev_action {
						
						Input::Multiply | Input::Divide => {with_brackets = false;},
			...
			// self.do_step(&action, with_brackets);
			
			self.do_step(&action, true);
			
			/* match action {
				
				Input::Add | Input::Subtract | Input::Multiply | Input::Divide => {prev_action = action.clone();},
...
//	Action show variables
	fn show_variables(&mut self) {
		...
		println!("command buffer={}", self.save_line);
		println!("program buffer={}", self.save_file_contents);
		print!("\n> ");
		io::stdout().flush().unwrap();
	}
	...
	fn do_step(&mut self, action: &Input, with_brackets: bool){
		
		self.step_resultado(action); 
		
		self.step_expresion(action, with_brackets);
	...
	fn step_expresion(&mut self, act: &Input, with_brackets: bool){
		match act {
			&Input::Add => {
				...
				// }
				// else {
					// self.expresion.push(format!("{}+{}", &prev, &last)); // fix this
				// }
			},
			&Input::Subtract => {
				let last = self.expresion.pop().expect("Algo no anduvo");
					...
					if command.find(',') == None {
						...
						if let Some(instruction_name) = &instruction.get(0) {
							match parse_instruction(&instruction_name.to_string()) {
								...
								// prcess program file prf,filename    
